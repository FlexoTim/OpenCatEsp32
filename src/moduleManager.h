...
// PTH("* " + source, long(millis() - lastSerialTime));
		if (!(token >= 'A' && token <= 'Z') || token == 'X' || token == 'R' || token == 'W') {  // serial monitor is used to send lower cased tokens by users
			...
			// if the serial monitor sends line ending
			// symbols
			// allow space between token and parameters,
			// such as "k sit"
			leftTrimSpaces(newCmd);
			for (int i = cmdLen - 1; i >= 0; i--) {
				// delete the '/r' and '/n' if the serial
				// monitor is configured to send
				// terminators
				if ((newCmd[i] == '\n') || (newCmd[i] == '\r')) {
					newCmd[i] = '\0';
					cmdLen--;
		...
		cmdLen = (newCmd[cmdLen - 1] == terminator)
			? cmdLen - 1
			: cmdLen;
